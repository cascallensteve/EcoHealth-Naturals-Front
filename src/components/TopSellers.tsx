...
export default function TopSellers() {
  ...
  const seq = Array.from({ length: Math.min(visible, items.length) }, (_, i) => items[(start + i) % items.length])

  return (
    <section className="mx-auto max-w-7xl px-6 pt-4 pb-10">
      <div className="flex items-center justify-between">
        <h2 className="text-xl sm:text-2xl font-semibold tracking-wide text-gray-900">TOP SELLERS RIGHT NOW</h2>
      ...
      <div className="mt-4 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {seq.map((p) => {
          const q = qty[p.id] || 1
          const badge = p.bottle ? 'RETURNABLE BOTTLE' : p.top ? 'TOP SELLER!' : ''
          return (
            <article key={p.id} className="overflow-hidden rounded-xl border bg-white shadow-sm ring-1 ring-black/5">
              <Link to={`/product/${slugify(p.title)}`} className="block">
                <div className="relative h-44 w-full overflow-hidden">
                  {badge && (
                    <span className={`absolute left-2 top-2 inline-flex rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase text-white ${p.bottle ? 'bg-emerald-700' : 'bg-emerald-600'}`}>{badge}</span>
                  )}
                  <img
                    src={p.img}
                    alt={p.title}
              ...
              <div className="px-3 pb-3 pt-2">
                <div className="flex items-center justify-between text-[11px]">
                  ...
                  <button
                    ...
                    onClick={() => cart.addItem({ id: p.id, title: p.title, price: parsePrice(p.price), img: p.img }, 1)}
                  >
                    ⚡ Add to Quick Shop
                  </button>
                </div>
                <h3 className="mt-2 line-clamp-2 text-[13px] font-medium text-gray-900">{p.title}</h3>
                <div className="mt-1 text-xs text-gray-500">KShs {parsePrice(p.price)}</div>
                ...
                <div className="mt-3 flex items-center justify-between">
                  ...
                  <button
                    ...
                    onClick={() => cart.addItem({ id: p.id, title: p.title, price: parsePrice(p.price), img: p.img }, q)}
                  >
                    ADD TO CART
                  </button>
                </div>
